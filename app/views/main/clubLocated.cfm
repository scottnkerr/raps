...
populateSelect();
$('#club_located_id').change(function() {
		var id = $(this).val();	
		$(":input[type='checkbox']").prop("checked",false);
		$(":input[type='checkbox']").wijcheckbox("refresh");
	if (id == 0) {
	    $('#limitsform')[0].reset();
		$('#club_located_id').val(0);
	...
	$.ajax({
		type: "GET",
		url: "/index.cfm?event=main.getClubLocated",
		dataType: "json",
		data: {
			club_located_id: id
		}
		...
		$.each(response.data, function(k,v) {	
			...
			$('#club_located').val(v.club_located);
	...
	var validator = $("#limitsform").validate({
		...
		submitHandler: function() {
			...
			$.ajax({
			  type: "POST",
			  cache: false,
			  url: "/index.cfm?event=main.saveClubLocated",
			  data: data
...
$('#limitsform')[0].reset();
$(":input[type='checkbox']").wijcheckbox("refresh");
$('#club_located_id').find('option').remove().end().append('<option value="0">Add new</option>').val(0);
$.ajax({
	type: "GET",
	url: "/index.cfm?event=main.getClubLocated",
	dataType: "json"
	...
	var html = '';
    $.each(response.data, function(k,v) {
		if (v.club_located_id != undefined)
		{
        html += '<option value="' + v.club_located_id + '">' + v.club_located + '</option>';
		}
	});
	$('#club_located_id').append(html);			 
...
<div id="statusbar">
  <div id="pagename">Club Locations Administration</div>
...
<li><select name="club_located_id" id="club_located_id" class="selectbox2" style="width:158px;">
  </select>
  </li>
 <li class="clear"><label>&nbsp;</label></li> 
 <li class="clear"><label class="width-120">Name</label></li>
 <li><input type="text" name="club_located" id="club_located" class="width-150" /></li>
 <li class="clear"><label>&nbsp;</label></li> 
