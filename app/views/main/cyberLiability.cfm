...
$('#cyber_liability_amount_id').change(function() {
	...
	if (id == 0) {
	    $('#limitsform')[0].reset();
		$('#cyber_liability_amount_id').val(0);
	...
	$.ajax({
		type: "GET",
		url: "/index.cfm?event=main.getCyberLiability",
		dataType: "json",
		data: {
			cyber_liability_amount_id: id
		}
		...
		$.each(response.data, function(k,v) {	
			...
			$('#cyber_liability_amount').val(v.cyber_liability_amount);
...
//end cyber_liability_amount_id change

	var validator = $("#limitsform").validate({
		rules: {
			cyber_liability_amount: {
				required: true
				}
		...
		submitHandler: function() {
			...
			$.ajax({
			  type: "POST",
			  cache: false,
			  url: "/index.cfm?event=main.saveCyberLiability",
			  data: data
...
$('#limitsform')[0].reset();
$(":input[type='checkbox']").wijcheckbox("refresh");
$('#cyber_liability_amount_id').find('option').remove().end().append('<option value="0">Add new</option>').val(0);
$.ajax({
	type: "GET",
	url: "/index.cfm?event=main.getCyberLiability",
	dataType: "json"
	...
	var html = '';
    $.each(response.data, function(k,v) {
		if (v.cyber_liability_amount_id != undefined)
		{
        html += '<option value="' + v.cyber_liability_amount_id + '">' + v.cyber_liability_amount + '</option>';
		}
	});
	$('#cyber_liability_amount_id').append(html);			 
...
<div id="statusbar">
  <div id="pagename">Cyber Liability Administration</div>
...
<li><select name="cyber_liability_amount_id" id="cyber_liability_amount_id" class="selectbox2" style="width:158px;">
  </select>
  </li>
 <li class="clear"><label>&nbsp;</label></li> 
 <li class="clear"><label class="width-120">Name</label></li>
 <li><input type="text" name="cyber_liability_amount" id="cyber_liability_amount" class="width-150" /></li>
 <li class="clear"><label>&nbsp;</label></li> 
