...
populateSelect();
$('#proposal_doc_list_id').change(function() {
		var id = $(this).val();	
		$(":input[type='checkbox']").prop("checked",false);
	if (id == 0) {
	    $('#limitsform')[0].reset();
		$('#proposal_doc_list_id').val(0);
	...
	$.ajax({
		type: "GET",
		url: "/index.cfm?event=main.getProposalDocLists",
		dataType: "json",
		data: {
			proposal_doc_list_id: id
		}
...
//end proposal_doc_list_id change
	...
	var validator = $("#limitsform").validate({
        ...
        var proposal_doc_list_id = $('#proposal_doc_list_id').val();
 ...
 //       disabled=
			...
			$.ajax({
			  ...
			  data: {
				  proposal_doc_list_id:proposal_doc_list_id,
...
$('#deletePD').click(function(e) {
	...
	thisid = $('#proposal_doc_list_id').val();
...
var currentselect = $('#proposal_doc_list_id').val();
$('#limitsform')[0].reset();
$(":input[type='checkbox']").wijcheckbox("refresh");
$('#proposal_doc_list_id').find('option').remove().end().append('<option value="0">Add new</option>').val(0);
...
$.ajax({
	...
	var html = '';
    $.each(response.data, function(k,v) {
		if (v.proposal_doc_list_id != undefined)
		{
        html += '<option value="' + v.proposal_doc_list_id + '">' + v.proposal_doc_name + '</option>';
		}
	});
	$('#proposal_doc_list_id').append(html);			 
	$('#proposal_doc_list_id').val(currentselect).trigger('change');		
});
...
<li><select name="proposal_doc_list_id" id="proposal_doc_list_id" class="selectbox2 docform" style="width:158px;">
  </select>
...
<li class="clear">
 <input type="submit" name="typesub" class="buttons" value="SAVE"> <button name="preview" id="preview" class="buttons">PREVIEW</button> <button name="deletePD" id="deletePD" class="buttons">DELETE</button></li>
 </ul> 
</form>
<div>
<div id="pdfembed"></div>
